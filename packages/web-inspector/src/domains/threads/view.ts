...
export interface ThreadsOverviewViewModel {
  ...
  copyIcon: unknown;
  onCopySetupPrompt: (event: Event) => void;
}
        ...
        <div class="cpk-threads-overview-actions">
          ${
            model.locked
              ? html`
                ${
                  model.licenseState === "valid"
                    ? html`
                      <button
                        data-inspector-threads-setup-prompt
                        type="button"
                        aria-label=${
                          state.setupPromptCopyState === "copied"
                            ? "Setup prompt copied"
                            : state.setupPromptCopyState === "error"
                              ? "Copy setup prompt failed. Try again"
                              : "Copy setup prompt for your coding agent"
                        }
                        @click=${model.onCopySetupPrompt}
                      >
                        ${model.copyIcon}
                        ${
                          state.setupPromptCopyState === "copied"
