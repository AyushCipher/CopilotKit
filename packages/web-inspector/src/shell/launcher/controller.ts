...
export class LauncherController {
  ...
  closeHud(): void {
    ...
    if (!this.state.hudOpen && this.state.hudHelp === null) return;
    this.state.hudOpen = false;
    this.state.hudHelp = null;
    this.host.requestUpdate();
  }
  ...
  readonly handleHudHelpClick = (event: Event, row: LauncherHudRowId): void => {
    event.preventDefault();
    event.stopPropagation();
    this.state.hudHelp = this.state.hudHelp === row ? null : row;
    this.host.requestUpdate();
  };
