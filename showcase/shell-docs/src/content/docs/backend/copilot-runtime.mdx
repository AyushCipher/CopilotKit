...
The examples below take `identifyUser` as given and create a
`CopilotKitIntelligence` client. See [Connect your runtime to Intelligence](/premium/connect-your-runtime)
for the project API key.

#### Assign Threads to Learning Containers

Create a Learning Container in your Intelligence Project, then return its
stable ID from the Intelligence client:
...
const intelligence = new CopilotKitIntelligence({
  apiKey: process.env.INTELLIGENCE_API_KEY!,
  getLearningContainerId: async ({ surface, agentId, userId, threadId }) => {
